...
class CrossAttentionFusion(nn.Module):
    ...
    def __init__(self, img_dim, spatial_dim, latent_dim, num_heads=4, dropout=0.1):
        ...
        # 4. 最终输出头，用于生成 VAE 的均值和对数方差
        self.fc_mu = nn.Linear(latent_dim, latent_dim)
        self.fc_log_var = nn.Linear(latent_dim, latent_dim)

    def forward(self, img_features, spatial_features):
        # 预处理：增加一个序列维度，因为Attention层需要 (batch, seq_len, dim)
        # img_features: [batch, 768] -> [batch, 1, 768]
        # spatial_features: [batch, 128] -> [batch, 1, 128]
        img_features = img_features.unsqueeze(1)
        spatial_features = spatial_features.unsqueeze(1)
        
        # 1. 投影到相同维度
        img_proj = self.img_proj(img_features)       # -> [batch, 1, latent_dim]
        spatial_proj = self.spatial_proj(spatial_features) # -> [batch, 1, latent_dim]
        
        # 2. 执行交叉注意力
        # query=空间, key=图像, value=图像
        ...
        # 3. Add & Norm + Feed Forward
        fused_features = self.norm1(spatial_proj + attn_output) # 残差连接
        fused_features_ff = self.mlp(fused_features)
        fused_features = self.norm2(fused_features + fused_features_ff) # 第二个残差连接
        
        # 移除序列维度
        fused_features = fused_features.squeeze(1) # -> [batch, latent_dim]
        
        # 4. 计算融合后的 mu 和 log_var
...
class MultiModalVAE(nn.Module):
    def __init__(self, input_channels=3, spatial_dim=2, latent_dim=128, img_size=32, rna_dim=2000, hidden_dims=None):
        ...
        # self.fusion_method = fusion_method # *** NEW: 保存融合方法 ***
        # --- 编码器 (Encoders) ---
